...
class LowLevelSafeController:
    """
    Low-level control chain:
    obs_low + skill -> deterministic H/F -> soft-CBF/soft-CLF QP -> safe action.
    """
    ...
    def _infer_qp_param(self, obs_low: AgentObsLow, skill_id: int) -> QPParam:
        ...
        if torch is not None and hasattr(self.low_policy, "forward"):
            ...
            return QPParam(
                H_mat=self._to_numpy(qp_param.H_mat).reshape(2, 2),
                f_lin=self._to_numpy(qp_param.f_lin).reshape(-1),
                w_clf=self._to_numpy(qp_param.w_clf).reshape(-1),
                w_cbf=self._to_numpy(qp_param.w_cbf).reshape(-1),
                cbf_slack_max=self._to_numpy(qp_param.cbf_slack_max).reshape(-1),
