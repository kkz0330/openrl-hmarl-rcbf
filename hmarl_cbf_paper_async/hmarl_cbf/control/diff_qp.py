...
def _robust_margin_from_a_row(a_row: Tensor, bound: float, enabled: bool, dtype: Any, device: torch.device) -> Tensor:
...
class TorchDifferentiableQPSolver:
    ...
    def solve(self, qp_param: QPParam, constants: DiffConstraintConstants) -> DiffQPSolveResult:
        ...
        b_cbf = b_cbf - constants.cbf_robust_margin
...
def build_diff_constraint_constants(
    ...
    if len(A_rows) == 0:
        ...
        cbf_robust_margin = torch.zeros((1,), dtype=dtype, device=dev)
    else:
    ...
    return DiffConstraintConstants(
        ...
        cbf_h0dot=cbf_h0dot,
        cbf_resp=cbf_resp,
        cbf_robust_margin=cbf_robust_margin,
        A_clf=A_clf,
        clf_V=clf_V,
