...
def _frame_lidar_points_from_adapter_info(adapter_info: Dict[int, Dict[str, Any]]) -> np.ndarray:
...
def _frame_lidar_obstacles_from_adapter_info(adapter_info: Dict[int, Dict[str, Any]]) -> List[Dict[str, Any]]:
...
def _build_agents_and_env(cfg: Dict[str, Any], args: argparse.Namespace):
    ...
    return core, env, executor, low_agent, high_agent
...
def _evaluate_episode(
    ...
    *,
    ...
    while env.agents:
        obs, rewards, terminations, truncations, infos, action_out = high_agent.step_env(
            env,
            obs,
            infos,
            deterministic=deterministic,
        )
        skill_switches += int(len(action_out))

        for step_record in env.last_low_step_records:
            physical_steps += 1
            unsafe_flags = dict(step_record["unsafe_flags"])
            reach_flags = dict(step_record["reach_flags"])
            safety_metrics = dict(step_record.get("safety_metrics", {}))
            adapter_info = dict(step_record.get("adapter_info", {}))
            rewards_step = dict(step_record.get("rewards", {}))
            ...
            for aid in agent_ids:
                ...
                returns_by_agent[aid] += float(rewards_step.get(aid, 0.0))
                min_h_agent = min(min_h_agent, float(safety_metrics.get(aid, {}).get("min_h_agent", 0.0)))
                min_h_obstacle = min(min_h_obstacle, float(safety_metrics.get(aid, {}).get("min_h_obstacle", 0.0)))

                info = dict(adapter_info.get(aid, {}))
                if info:
                    qp_count += 1
                    qp_feasible_count += int(bool(info.get("qp_feasible", False)))
                    qp_fallback_count += int(bool(info.get("qp_used_fallback", False)))
                    slack = np.asarray(info.get("qp_slack", []), dtype=np.float32).reshape(-1)
                    cbf_slack = np.asarray(info.get("qp_cbf_slack", []), dtype=np.float32).reshape(-1)
                    if slack.size > 0:
            ...
            if capture_trace:
                frame_lidar_obstacles = _frame_lidar_obstacles_from_adapter_info(adapter_info)
                trace_positions.append(
                    np.stack(
                        [np.asarray(step_record["positions"][aid], dtype=np.float32).reshape(2) for aid in agent_ids],
                        axis=0,
                    )
                )
                trace_unsafe.append(
                    np.asarray([bool(unsafe_flags.get(aid, False)) for aid in agent_ids], dtype=bool)
                )
                wind = np.asarray(step_record.get("wind_accel", np.zeros(2, dtype=np.float32)), dtype=np.float32).reshape(2)
                frame_labels.append(
                    f"t={int(step_record.get('step_count', physical_steps))} "
                    f"wind=({wind[0]:+0.2f},{wind[1]:+0.2f}) "
                    f"{_summarize_lidar_obstacle_types(frame_lidar_obstacles)}"
                )
                trace_lidar_points.append(_frame_lidar_points_from_adapter_info(adapter_info))
...
def evaluate_stack(cfg: Dict[str, Any], args: argparse.Namespace) -> Dict[str, Any]:
    _, env, executor, _, high_agent = _build_agents_and_env(cfg, args)
    ...
    summary = {
        ...
        "deterministic": bool(args.deterministic),
        "low_checkpoint": str(args.low_checkpoint),
        ...
        "avg_skill_switches": float(summary_stats.avg_skill_switches),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Unified evaluation for rebuilt OpenRL HMARL-CBF stack")
    parser.add_argument("--config", type=str, required=True)
    parser.add_argument("--low-checkpoint", type=str, required=False, default=None)
    parser.add_argument("--high-checkpoint", type=str, required=False, default=None)
...
def main() -> None:
    ...
    if not args.low_checkpoint or not args.high_checkpoint:
        raise ValueError("eval_openrl_stack requires low/high checkpoints from CLI or the openrl_eval config section")
    summary = evaluate_stack(cfg, args)
