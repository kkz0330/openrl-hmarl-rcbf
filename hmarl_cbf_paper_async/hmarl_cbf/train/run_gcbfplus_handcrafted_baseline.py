...
from hmarl_cbf.baselines.gcbfplus_adapter import GCBFPlusHandcraftedConfig, GCBFPlusHandcraftedController
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate original GCBF+ handcrafted CBF controller on the HMARL environment via an adapter."
    )
    ...
    parser.add_argument(
        "--debug-first-episode",
        action="store_true",
        help="Print fine-grained progress logs for the first episode to locate stalls.",
    )
    parser.add_argument(
        "--progress-interval",
        type=int,
        default=50,
        help="Print coarse progress every N steps. Set <=0 to disable.",
    )
    parser.add_argument(
        "--max-steps",
        type=int,
        default=0,
        help="If >0, stop each episode after this many environment steps for smoke testing.",
    )
    parser.add_argument(
        "--scenario",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Optional JSON array of obstacles. Use with --states-json for fixed scenes.",
    )
...
def _make_run_dir(output_root: Path, run_name: str) -> Path:
    if run_name:
        ...
        out = output_root / time.strftime("gcbfplus_baseline_%Y%m%d_%H%M%S")
...
def main() -> None:
    ...
    gcbf_cfg = GCBFPlusHandcraftedConfig.from_mapping(
        {
            "alpha": cfg.get("gcbfplus_handcrafted_baseline", {}).get("alpha", 1.0),
            "k": cfg.get("gcbfplus_handcrafted_baseline", {}).get("k", env_cfg.get("lidar_cbf_top_k", 3)),
            "action_limit": env_cfg["action_limit"],
            "velocity_limit": env_cfg["velocity_limit"],
            "comm_radius": cfg.get("gcbfplus_handcrafted_baseline", {}).get(
                "comm_radius",
            ...
            "mass": cfg.get("gcbfplus_handcrafted_baseline", {}).get("mass", 1.0),
            "dt": env_cfg["dt"],
            "world_size": env_cfg["world_size"],
            "horizon": env_cfg["horizon"],
        }
    )
    baseline = GCBFPlusHandcraftedController(gcbf_cfg)

    run_dir = _make_run_dir(Path(args.output_root), args.run_name)
    snapshot = dict(cfg)
    snapshot["gcbfplus_handcrafted_baseline"] = {
        "alpha": float(gcbf_cfg.alpha),
        "k": int(gcbf_cfg.k),
        "action_limit": float(gcbf_cfg.action_limit),
        "velocity_limit": float(gcbf_cfg.velocity_limit),
        "comm_radius": float(gcbf_cfg.comm_radius),
        "car_radius": float(gcbf_cfg.car_radius),
        "n_rays": int(gcbf_cfg.n_rays),
        "mass": float(gcbf_cfg.mass),
        "dt": float(gcbf_cfg.dt),
        "world_size": float(gcbf_cfg.world_size),
        "horizon": int(gcbf_cfg.horizon),
    }
    ...
    for ep in range(n_episodes):
        if args.debug_first_episode and ep == 0:
            print("[debug] episode 1: reset-start", flush=True)
        if fixed_scene is not None:
            obs, _ = env.reset(seed=seed + ep, states=fixed_scene["states"], obstacles=fixed_scene["obstacles"])
        else:
            obs, _ = env.reset(seed=seed + ep)
        if args.debug_first_episode and ep == 0:
            print("[debug] episode 1: reset-done", flush=True)
        ...
        while not (terminated or truncated):
            if args.debug_first_episode and ep == 0 and step_idx < 3:
                print(f"[debug] episode 1 step {step_idx}: state-collect", flush=True)
            states_map = {s.agent_id: s for s in env.get_agent_states()}
            obs_low_map = {aid: obs[aid]["low"] for aid in agent_ids}
            if args.debug_first_episode and ep == 0 and step_idx < 3:
                print(f"[debug] episode 1 step {step_idx}: baseline-act-start", flush=True)
            actions, relax = baseline.act(states=states_map, obs_low=obs_low_map)
            if args.debug_first_episode and ep == 0 and step_idx < 3:
                print(f"[debug] episode 1 step {step_idx}: baseline-act-done", flush=True)
            obs, rewards, terminated, truncated, info = env.step(actions)
            if args.debug_first_episode and ep == 0 and step_idx < 3:
                print(f"[debug] episode 1 step {step_idx}: env-step-done", flush=True)
            ...
            if int(args.progress_interval) > 0 and (step_idx % int(args.progress_interval) == 0):
                mean_relax = float(
                    np.mean(
                        [
                            float(np.mean(np.asarray(relax[aid], dtype=np.float32).reshape(-1)))
                            if np.asarray(relax[aid], dtype=np.float32).size > 0
                            else 0.0
                            for aid in agent_ids
                        ]
                    )
            ...
            if int(args.max_steps) > 0 and step_idx >= int(args.max_steps):
                truncated = True
                print(
                    f"[progress] episode {ep + 1}/{n_episodes} reached --max-steps={int(args.max_steps)}; truncating for smoke test",
                    flush=True,
                )
                break
    ...
    print("==== GCBF+ Handcrafted Baseline Summary ====")
